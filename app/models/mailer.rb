...
#  FROM = 'noreply@talks.cam.ac.uk'
  ...
  def daily_list( subscription )
    ...
    @subject    = "[" + $inst.talksSiteName + "] Today's talks: #{subscription.list.name}"
  ...
  def weekly_list( subscription )
    ...
    set_common_variables( subscription )
    @subject    = "[" + $inst.talksSiteName + "] This week's talks: #{subscription.list.name}"
  ...
  def talk_tickle( tickle )
    talk = tickle.about
    @subject = "[" + $inst.talksSiteName + "] A talk that you might be interested in"
    @body = { :tickle => tickle,
      :talk => talk,
  ...
  def list_tickle( tickle )
    ...
    @subject = "[" + $inst.talksSiteName + "] A list that you might be interested in"
    @body = { :tickle => tickle,
      :list => list,
      :talks => talks,
